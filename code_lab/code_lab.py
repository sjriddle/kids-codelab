...
class Song:
    ...
    def play(self, note, beats=1):
        self.notes.append(
            {"note": str(note), "freq": note_to_freq(note), "beats": float(beats)}
        )
        self._guard()
...
class Stage:
    ...
    def current(self):
        return [
            {"emoji": s.emoji, "x": s.x, "y": s.y, "size": s.size} for s in self.sprites
        ]
...
def _friendly_error(etype, message, code):
    ...
    if etype == "NameError":
        ...
        return (
            ("I don't know the word '%s'. Check the spelling!" % name) if name else None
        )
    if etype == "AttributeError":
        ...
        if match:
            return "There's no command called '%s' — did you mean '%s'?" % (
                attr,
                match[0],
            )
        return (
            ("There's no command called '%s'. Check the spelling!" % attr)
            if attr
            else None
        )
    if etype == "TypeError":
    ...
    if coll["stages"]:
        ...
        if callable(ns.get("on_key")) or callable(ns.get("on_click")):
            ...
            base.update(
                {
                    "kind": "anim",
                    "frames": stage.frames or [stage.current()],
                    "background": stage.background,
                }
            )
    elif coll["robots"]:
...
def run_event(sid, ev_type, key=None, x=None, y=None):
    ...
    if not sess:
        return {
            ...
            "error": {
                "type": "Gone",
                "message": "Press Run again to start playing.",
                "line": None,
            },
            "output": "",
        }
    ...
    try:
        ...
        error = {
            "type": "RuntimeError",
            "line": None,
            "message": "That took too long — maybe an endless loop? 🐢",
            "friendly": None,
        }
    except Exception as e:  # noqa: BLE001
...
LESSONS = [
    ...
    },
    {
        ...
        "emoji": "⭐",
        "goal": "Each note is a little box (a dictionary) with a 'note' and a 'count' of beats. Change a count, swap a note, or use 'rest' for a quiet beat!",
        "tip": "A dictionary keeps things together with labels — here every note has a 'note' and a 'count'. The loop reads each box; type 'rest' (or leave it empty '') to make a quiet beat instead of a sound.",
        "code": 'speed = 120\n\nsong = Song(speed)\n\n# Each note is a little dictionary: a "note" and a "count" of beats.\nmelody = [\n    {"note": "C", "count": 1},\n    {"note": "C", "count": 1},\n    {"note": "G", "count": 1},\n    {"note": "G", "count": 1},\n    {"note": "A", "count": 1},\n    {"note": "A", "count": 1},\n    {"note": "G", "count": 2},\n    {"note": "rest", "count": 1},\n    {"note": "F", "count": 1},\n    {"note": "F", "count": 1},\n    {"note": "E", "count": 1},\n    {"note": "E", "count": 1},\n    {"note": "D", "count": 1},\n    {"note": "D", "count": 1},\n    {"note": "C", "count": 2},\n]\n\nfor item in melody:\n    if item["note"] == "" or item["note"] == "rest":\n        song.rest(item["count"])\n    else:\n        song.play(item["note"], item["count"])\n',
    },
    {
        "track": "🎵 Music",
        "title": "Free music",
